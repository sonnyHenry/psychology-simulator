...
│   │       ├── narrative-slots/   # 叙事功能位定义(每位 ≥3 候选,跨局差异的骨架)
...
interface Citation {
  ...
  verified: boolean;                 // ← 已完成发布采用的核对流程，并在 LEDGER 留痕
}
```
...
GAME_DESIGN 20.3② 的实现。这主要是**内容组织方式的改变**；`GameState` 只记录已经实际播放过的
`filledSlots`,功能位定义本身仍留在内容包中，不成为跨年对象。
...
> **M7.5 实测（300 局，seed 42–341）**:渲染三元组 2.4%、去 mandatory 的事件 ID 重合 24.5%、
> 连续 3 局独有 55.5%、本配置可达库覆盖 27.2%、相同 / 不同配置差 33.0pp；全部门禁通过。
> 全内容库覆盖 14.8% 作为诊断值保留，不用于验收，因为领域 / 导师 / 取向 / 路径内容彼此互斥。
...
> **进度**:M0 / M1 / M2 / M2.5 / M3(含 M3.1–M3.3 实机修复)/ M3.5 / M3.6 / M4 / M4.6 / M4.5 / M5 / M6 / M7 / M7.5 已完成。逐轮的做了什么、抓到了什么 bug、接手要注意什么,
...
| **M8 打磨** | 内容填充至约 471 事件 / 34–38 结局 + `repetition` 驱动变体补齐 + 术语与时代一致性校验全绿 + 移动端适配 | 全门禁绿；GitHub Pages 自动发布 |

> **M8 落地（内容包 `0.8.0`）**:471 事件 / 37 结局；新增 80 个配置专属场景与 30 幕 NPC 关系链；
> 术语表、年代及执业边界进入 validate；叙事功能位支持 `pool` / `conditional` 两种候选口径；
> 375px 与 320px 窄屏无横向溢出、主要触控目标 ≥44px。`typecheck`、152 项 core 测试、70 条 validate
> 反例、16 个跳转、300 局重复度、3000+1500 局模拟与 web 构建全部通过。项目维护者决定人工文献二次复核
> 不作为发布门槛；台账仍必须如实保留机器初核与人工栏空白的区别。
