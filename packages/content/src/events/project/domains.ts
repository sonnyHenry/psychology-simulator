...
const scenes: DomainScenes[] = [
  {
    domain: 'cognition',
    ideation: { title: '实验做法比问题更有名', text: '「{{project}}」最初是从一套经典实验做法长出来的。{{advisor}}问：如果不提那套实验的名字，你真正想知道什么？', choice: '先把问题写成人话', result: '你删掉三个术语，终于剩下一句能被数据判断对错的问题。' },
    collect: [
      { title: '反应时快得不像真的', text: '「{{project}}」第 {{years}} 年。第一批数据里有人每道题都在 120 毫秒内作答，正确率却接近满分。', choice: '按预先规则剔除', result: '样本少了一截，剩下的反应时终于像人做出来的。' },
      { title: '实验顺序盖过了真正差异', text: '「{{project}}」跑到一半，你发现先做困难任务的人后面普遍更慢。原本用来平衡先后顺序的安排表漏了一种顺序。', choice: '补齐缺的顺序', result: '你又招了一批人。设计上的洞，比统计模型更早被补上。' },
    ],
    analyze: [
      { title: '少数特别慢的反应拖动了平均数', text: '「{{project}}」的大多数反应时间挤在一起，少数人却慢很多。原计划比较平均数，会被这几个特别慢的反应牵着走。', choice: '换一种适合这批数据的比较方法', result: '结论变窄了，也更像这批数据真正允许你说的话。' },
      { title: '快和准不能分开看', text: '一个条件更快，也更容易错。只报告反应时，故事很好看；把正确率放进来，故事变成了速度—准确权衡。', choice: '把两条结果一起报告', result: '摘要没有原来利落，但不再拿半个现象冒充全部。' },
    ],
    write: { title: '不要把一次任务写成整个心智', text: '写「{{project}}」讨论时，{{advisor}}圈出一句：“这个任务测到的，不等于这个能力的全部。”', choice: '缩小结论适用的范围', result: '你删掉“揭示了人类认知机制”，换成了这套任务真正支持的范围。' },
  },
  {
    domain: 'cogneuro',
    ...
    analyze: [
      { title: '两套数据清理方法，两张不同的脑图', text: '「{{project}}」只换了一套去噪参数，原本在左侧亮起的脑区就跑到了右侧。两种处理方法都能找到论文支持。', choice: '按收数据前定好的方法，并公开另一种结果', result: '主图只留一张，补充材料诚实地放下另一张。' },
      { title: '十万个小区域都可能碰巧亮起来', text: '脑图同时比较了十万个小区域。把“偶然亮起”的风险校正后，最漂亮的那一块不见了。{{advisor}}盯着空下来的图例没有说话。', choice: '保留校正后的结果', result: '图不再适合做海报封面，但错误率终于有了边界。' },
    ],
    write: { title: '脑图亮起来不等于它“负责”一切', text: '「{{project}}」的讨论里写着某脑区“负责”一种情绪。脑成像信号只说明活动与任务同时变化，没有说过这么强的话。', choice: '改成“有关联”和“存在任务差异”', result: '动词变弱了，论证反而站得更稳。' },
  },
  {
    domain: 'social',
    ideation: { title: '三个量表和两条因果箭头', text: '「{{project}}」的第一版模型里有三个量表和两条箭头，假设 A 先改变 B，再通过 B 影响 C。{{advisor}}问哪一条有理论依据，哪一条只是因为软件能算。', choice: '删掉讲不清原因的箭头', result: '模型少了一半，看起来第一次像一个问题而不是一张路径图。' },
    collect: [
    ...
    analyze: [
      { title: '同一张问卷会让关系看起来更强', text: '原因和结果都在同一张问卷、同一分钟里填写。它们的关系很漂亮，却可能只是相同题型和作答习惯造成的。', choice: '把这种测量限制写清楚', result: '你没有假装同一时间测到的相关已经解释了谁导致谁。' },
      { title: '人都在班级里面', text: '「{{project}}」把八所学校的学生摊平成独立个体。班级内相关不高，但绝不是零。', choice: '改用多层模型', result: '标准误变大，一条边不再显著；学校与班级第一次进入模型。' },
    ],
    write: { title: '一起变化不等于谁导致谁', text: '初稿三次把“同一时间测到的相关”写成“影响”。{{advisor}}只批了四个字：时间顺序呢？', choice: '逐句收回“导致”和“影响”', result: '文章少了一个宏大故事，多了一条清楚的证据边界。' },
  },
  {
    domain: 'clinical',
  ...
  },
  {
    ...
    ideation: { title: '量表不是把相似题目加起来', text: '「{{project}}」的题目都很像，所以答案看起来会非常一致。{{advisor}}问：你真正想测的心理特征有哪几面，这些题目都覆盖了吗？', choice: '先画出要测的内容地图', result: '你删掉重复题，也补上原来完全没有问到的一面。' },
    collect: [
    ...
    write: { title: '答案很一致，不等于真的测对了', text: '「{{project}}」摘要写着：“题目答案非常一致（α=.94），因此量表高度有效。”{{advisor}}把“因此有效”划掉：一致只能说明测得稳定，不能单独证明测到了目标。', choice: '把“测得稳定”和“测得准确”的证据分开写', result: '一个系数回到它能证明的那件事上。' },
  },
  {
    domain: 'health',
