 ...
 * 被拒这么多次,这个课题就不再投了。
 *
 * ## ⚠️ 这条规则目前几乎不开火,原因不在阈值
 *
 * 3000 局里 `rejections` 的分布是 `{0: 323, 1: 1}`——**没有一个课题真的被拒死过。**
 * 原因是 `review` 是管线的最后一站:课题一旦在这一站推进成功就直接发表,
 * 而"这一年一次都没推进成功"才算被拒一次。以现在的成功率,
 * 课题基本都是**到达 review 的那一年就发出去了**,从来不会在这一站停留过年。
 *
 * 于是"投了四个刊都没中"——现实里最常见的一种做废——在游戏里等于不存在。
 * 这跟 M3 那个"817 次伦理审查、0 次收数据"是同一个病:
 * **管线只是路过的那一站,它的内容和机制都不会发生。**
 *
 ...
 * 做了这么多年还没写出来的课题,也算烂了。
 *
 * **原来写的是 6,而那是一条死规则**:硕博全程一共就 6 年(2019–2024),
 * 一个研二开的课题永远够不到这个数,于是这条判定一次都没生效过。
 * 改成 4 之后它真的开始杀课题了(随机 bot 的做废率 70.3% → 74.4%),
 * 而且四年没进入写作确实就是死了——这不是为了配平凑的数。
 *
 * > 教训和 M3 那条"上限 + 大量掷骰 = 确定机制"是同一类:
 * > **每个阈值都要放回时间线里核一遍够不够得到。** 静态检查查不出死常量,
 * > 只有统计能——所以每个阈值都该有一条 simulate 指标盯着它。
 */
const MAX_YEARS_BEFORE_WRITING = 4;
