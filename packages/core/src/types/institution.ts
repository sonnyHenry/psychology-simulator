...
export interface GameifiedTerms {
  ...
  admission?: {
    ...
    funding?: string;
    /**
     * **毕业硬指标**(M4.6,GAME_DESIGN 4.3)。展示文案,常驻在工作台顶部。
     *
     * 这条数据让"我当年去了哪"这件事**每年都在你眼前**:A+ 校要三篇,双非要一篇,
     * 后者可能三年就毕业,前者第六年还在改稿。此前院校的差异只在录取那一屏出现过一次,
     * 之后就消失了。
     *
     * **只列清单,不算总分。** 把发表折算成综合积分是清晰的游戏化设计,
     * 但这一行真实的说法就是"两篇 SCI 其中一篇二区",折成分数反而不像话。
     */
    graduationBar?: string;
    /**
     * 上面那句话的结构化版本。**两者必须一致**(validate 规则 34):
     * 要么都有要么都无,而且数字必须在 `graduationBar` 文案里出现。
     * 两份数据说两件事,是这一行最容易写出来又最难发现的错。
     */
    graduationReq?: {
      /** 总篇数下限 */
      papers: number;
